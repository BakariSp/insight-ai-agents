...
- [ ] **1.1.1** 安装 FastAPI + uvicorn + sse-starlette 依赖，更新 `requirements.txt`
- [ ] **1.1.2** 创建 `main.py` FastAPI 入口，配置 CORS 中间件
- [ ] **1.1.3** 迁移 `/health` 端点 → `api/health.py` (GET `/api/health`)
- [ ] **1.1.4** 迁移 `/chat` 端点 → 临时保留为兼容路由（Phase 4 替换）
- [ ] **1.1.5** 迁移 `/models` 和 `/skills` 端点
- [ ] **1.1.6** 删除 `app.py`（Flask 入口），确认 `python main.py` 可正常启动
- [ ] **1.1.7** 更新测试：`test_app.py` 改用 `httpx.AsyncClient` + FastAPI TestClient
...
- [ ] **1.2.1** 创建 `config/settings.py`：`Settings(BaseSettings)` 定义所有配置项
  - 服务配置: `service_port`, `cors_origins`, `debug`
  - LLM 配置: `default_model`, `executor_model`, API keys
  - 功能配置: `brave_api_key`, `memory_dir`
- [ ] **1.2.2** 实现 `get_settings()` 单例函数（`@lru_cache`）
- [ ] **1.2.3** 迁移 `config.py` 中的所有配置到新 Settings，删除旧 `config.py`
- [ ] **1.2.4** 创建 `.env.example` 模板，列出所有环境变量
...
- [ ] **1.3.1** 创建 `models/base.py`：`CamelModel` 基类（`alias_generator=to_camel`, `populate_by_name=True`）
- [ ] **1.3.2** 创建 `models/blueprint.py`：
...
- [ ] **1.3.3** 创建 `models/request.py`：API 请求/响应模型
  - `WorkflowGenerateRequest/Response`
  - `ReportGenerateRequest`
  - `ReportChatRequest/Response`
- [ ] **1.3.4** 编写 Blueprint 模型单元测试：验证 camelCase 序列化、嵌套结构、路径引用字段
...
- [ ] **1.4.1** 创建 `config/component_registry.py`：定义 6 种组件（kpi_grid, chart, table, markdown, suggestion_list, question_generator）
- [ ] **1.4.2** 每种组件包含: `description`, `data_shape`, `props`, `variants`（如适用）
- [ ] **1.4.3** 编写辅助函数：`get_registry_description()` → 供 system prompt 注入
...
- [ ] **1.5.1** 安装 fastmcp 依赖
- [ ] **1.5.2** 创建 `tools/__init__.py`：实例化 `mcp = FastMCP("insight-ai-tools")`
- [ ] **1.5.3** 创建 `tools/data_tools.py`：4 个数据工具（mock 版本）
...
- [ ] **1.5.4** 创建 `tools/stats_tools.py`：2 个统计工具
...
- [ ] **1.5.5** 创建 `services/mock_data.py`：集中管理 mock 数据（班级、学生、成绩样本）
- [ ] **1.5.6** 编写工具单元测试：验证每个 `@mcp.tool` 的输入输出
...
- [ ] Blueprint model camelCase 序列化正确
- [ ] 6 个 FastMCP 工具可通过 `fastmcp dev` 测试
- [ ] `pytest tests/ -v` 全部通过
...
**目标**: 实现 ExecutorAgent，接收 Blueprint 执行三阶段流水线（Data → Compute → Compose），通过 SSE 流式输出报告。这是"可执行计划 → 实际报告"的核心环节。
...
- [ ] **3.3.1** 创建 `api/report.py`：`POST /api/report/generate`
...
- [ ] **3.3.4** 在 `main.py` 注册 report router

> ✅ 验收: `curl -N -X POST /api/report/generate` 收到 SSE 事件流，最终 `COMPLETE` 事件包含完整报告。
...
- [ ] **3.4.2** 验证报告内容：KPI 数值来自 tool 计算（可信），叙事文本来自 AI（基于数据）
- [ ] **3.4.3** 验证 SSE 事件格式符合 [sse-protocol.md](api/sse-protocol.md) 规范

> ✅ 验收: 完整流程可跑通，SSE 输出符合协议，报告结构匹配 Blueprint 的 UIComposition。

---

## Phase 4: Router + Chat 🔲

**目标**: 实现 RouterAgent（意图分类）和 ChatAgent（报告追问对话），完成多 Agent 协作闭环。用户可以在报告生成后继续追问、修改或重新生成。

**前置条件**: Phase 3 完成（ExecutorAgent 可执行 Blueprint 并 SSE 输出报告）。
...
- [ ] **4.1.1** 创建 `config/prompts/router.py`：意图分类 system prompt
  - 定义三种意图：`workflow_rebuild` / `report_refine` / `data_chat`
  - 包含分类示例和判断规则
- [ ] **4.1.2** 创建 `agents/router.py`：`RouterAgent`
  - 输入：用户消息 + 当前 workflow 名称 + 报告摘要
...
### Step 4.2: ChatAgent (报告对话)

> 基于已有报告上下文回答用户追问。

- [ ] **4.2.1** 创建 `config/prompts/chat.py`：对话 system prompt
- [ ] **4.2.2** 创建 `agents/chat.py`：`ChatAgent`
  - 输入：用户消息 + 报告上下文（摘要 + 关键数据）
  - 输出：Markdown 格式文本回复
- [ ] **4.2.3** 在 `api/report.py` 添加 `POST /api/report/chat` 端点
- [ ] **4.2.4** 编写对话测试：验证回复与报告上下文相关

> ✅ 验收: 给定报告上下文和追问，返回有意义的回复，不产生幻觉数据。
...
- [ ] **4.3.2** 联调测试：生成报告 → 追问 → Router 分类 → 对应 Agent 处理
...
- [ ] **6.1.1** 协调前端创建 proxy routes（参见 [前端集成文档](integration/frontend-integration.md)）：
  ...
  - `/api/ai/report-generate` → `POST /api/report/generate` (SSE passthrough)
  - `/api/ai/report-chat` → `POST /api/report/chat`
...
- [ ] **6.2.4** 前端适配：ReportRenderer 处理 `aiContentSlot` 标记的组件

> ✅ 验收: 报告中 markdown / suggestion_list 等组件内容由 AI 实时生成并流式推送。
...
- [ ] **6.3.1** 编写 E2E 测试用例：
  - 正常流程：输入 prompt → 生成 Blueprint → 执行报告 → 追问 → 对话
  - 异常流程：Java 超时降级、LLM 不可用、无效 prompt
- [ ] **6.3.2** 性能基线：记录 Blueprint 生成耗时、报告执行耗时、SSE 首字节时间
...
> ✅ 验收: 生产环境部署成功，教师可通过前端完成完整的"提问 → 报告 → 追问"流程。
...
| **M3: 报告生成** | 3 | Blueprint → SSE 流式报告 |
| **M4: 多 Agent 闭环** | 4 | 生成 + 追问 + 路由，完整交互循环 |
